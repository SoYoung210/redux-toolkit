...
const features = [
  {
    content: (
      <p>
        <strong>스토어 설정, 리듀서 생성, 불변성 업데이트 로직</strong>
        사용을 편리하게 해주는 유틸리티 포함
      </p>
    ),
    image: (
  ...
  },
  {
    content: (
      <p>
        <strong>store설정에 관한 기본 설정</strong>을 제공하며{' '}
        <strong>일반적으로 사용되는 Redux addon</strong>이 내장되어 있습니다.
      </p>
    ),
    image: (
  ...
  },
  {
    content: (
      <p>
        Immer와 Autodux와 같은 라이브러리에서 영감을 얻어{' '}
        <strong>'변경'로직으로 '불변성'로직을 작성할 수 있게 하고</strong>
        <strong>state 전체를 "slice"로 자동으로 만들 수 있습니다.</strong>
      </p>
    ),
    image: (
  ...
  },
  {
    content: (
      <p>
        어플리케이션에 필요한 핵심 로직에 집중할 수 있으므로{' '}
        <strong>적은 코드로 많은 작업을 수행할 수 있습니다.</strong>.
      </p>
    ),
    image: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
...
const otherLibraries = [
  {
    content: 'JavaScript 어플리케이션을 위한 예측 가능한 상태 컨테이너',
    title: 'Redux',
  ...
  {
    content: 'React에서 Redux를 사용하기 위한 공식 라이브러리',
    title: 'React-Redux',
